...
st.warning(
    ...
    "avertissements ⚠️ propres à chaque indicateur."
)
...
st.warning(
    "⚠️ **Petits volumes** : dans les communes à faible nombre de transactions, la médiane est "
    "peu fiable. Vérifiez le **nombre de ventes** affiché.\n\n"
    "⚠️ **Paris / Lyon / Marseille** : les transactions sont rattachées aux **arrondissements** "
...
st.warning("⚠️ Historique limité à **2021–2025** : la croissance « 5 ans » n'est calculable que si les deux années existent, sinon elle vaut 0.")

st.markdown("#### Loyer au m²")
st.markdown("Loyer **prédit** au m² (logement type), issu du modèle national « Carte des loyers ».")
st.warning("⚠️ C'est une **estimation modélisée**, pas un loyer réellement observé pour un bien donné. À considérer comme un ordre de grandeur.")
...
st.warning("⚠️ **Brute** : ne déduit ni charges, ni taxe foncière, ni vacance, ni fiscalité. Le rendement net réel est inférieur.")
...
st.warning("⚠️ Pour préserver le **secret statistique**, l'INSEE ne publie pas le revenu des très petites communes : la valeur peut être manquante.")

st.markdown("#### Taux de pauvreté")
st.error(
    "⚠️ **Indicateur DÉPARTEMENTAL.** Le taux de pauvreté disponible ici est celui du **département**, "
...
st.warning("⚠️ Ce sont des **décomptes bruts**, non rapportés à la population. Une grande ville aura mécaniquement plus d'équipements qu'un village.")
...
st.warning(
    "⚠️ **Composite** : un score unique masque des réalités contrastées (une commune peut être bien notée "
    "en transport mais mal en éducation).\n\n"
    "⚠️ **Pondération à confirmer** : la combinaison exacte des sous-scores est définie dans le pipeline "
    "amont et n'est pas redocumentée ici.\n\n"
    "⚠️ **Prix de repli** : pour les communes sans prix communal fiable (dont Paris/Lyon/Marseille), "
...
st.warning("⚠️ Un cluster est une **catégorie descriptive**, pas un classement de qualité.")
...
st.warning("⚠️ Calcul basé sur le **prix médian** de la zone : c'est un ordre de grandeur, pas le prix d'un bien réel. Frais de notaire, travaux et apport ne sont pas inclus.")
